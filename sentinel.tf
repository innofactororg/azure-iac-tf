...
module "sentinel_ar_scheduled" {
  ...
  entity_type                = try(each.value.entity_type, yamldecode(file("${path.cwd}/${each.value.definition_file}"))["properties"]["entityMappings"].entityType, null)
  identifier                 = try(each.value.identifier, yamldecode(file("${path.cwd}/${each.value.definition_file}"))["properties"]["entityMappings"].fieldMappings[0].identifier, null)
  column_name                = try(each.value.column_name, yamldecode(file("${path.cwd}/${each.value.definition_file}"))["properties"]["entityMappings"].fieldMappings[0].columnName, null)
}
